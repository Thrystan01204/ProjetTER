...
def encode_face(image):
    #detection du visage avec face_detector
    face_locations = face_detector(image, 1)
    face_encodings_list = []
    landmarks_list = []
    for face_location in face_locations:
        # DETECT FACES
        #mettre les points 68 coordonnées 
        shape = pose_predictor_68_point(image, face_location)
        #vecteur 128 dimentions qui sont dans la liste
...
def easy_face_reco(frame, known_face_encodings, known_face_names):
    ...
    for face_encoding in face_encodings_list:
        ...
        #comparateur de visage
        vectors = np.linalg.norm(known_face_encodings - face_encoding, axis=1)
        #tolérance 
        tolerance = 0.6
        result = []
        #si il a détecté le visage 
    ...
    #affichage du  text et rectangle 
